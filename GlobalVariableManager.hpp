...
struct GlobalVariableManager {
    std::map<std::string, std::vector<std::pair<std::string,std::string>>> var_to_argument_list; // func_var -> vector( pair(type, arg_name) )
    ...
    int get_type_size(std::string type) const;
};
